      ...
      {items.map((it) => {
        ...
        return (
          <li key={it.id} className="flex items-start gap-2 bg-card border rounded-lg p-2">
            <div
              className={`size-6 rounded-md flex items-center justify-center shrink-0 ${m.tone}`}
            >
              ...
              <div className="flex items-baseline gap-1.5">
                ...
                {it.kind === "promise" && it.status && (
                  <span className="text-[9px] text-muted-foreground">
                    ({PROMISE_AR[it.status] ?? it.status})
                  </span>
                )}
            ...
            {it.amount != null && (
              <span
                className={`font-extrabold text-[12px] tabular-nums shrink-0 ${isCredit ? "text-danger" : it.kind === "tx" ? "text-success" : ""}`}
              >
                {fmtMoney(it.amount)}
              </span>
            )}
