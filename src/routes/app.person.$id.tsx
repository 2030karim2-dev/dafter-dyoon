...
function PersonPage() {
  ...
  const [tab, setTab] = useState<
    "timeline" | "feed" | "activity" | "promises" | "attachments" | "insights"
  >("timeline");
  ...
  const load = async () => {
    ...
    setCompany(
      (co as { name: string | null; phone: string | null; address: string | null } | null) ?? null,
    );
    setLoading(false);
  };
  ...
  useEffect(() => {
    ...
    const next =
      currencies.find((c) => c.id === saved) ?? currencies.find((c) => c.is_base) ?? currencies[0];
  ...
  const scopedOpenings = useMemo(
    () => openings.filter((o) => o.currency_id === curId),
    [openings, curId],
  );
  const primaryBalance =
    balancesByCurrency.find((b) => b.currency.id === curId) ?? balancesByCurrency[0];
  ...
  const buildShareText = () => {
    ...
    lines.push("");
    lines.push(
      `نرفق لكم كشف حساب ${primaryBalance?.currency.name ?? ""} لدى *${companyName}* حتى تاريخ ${today}:`,
    );
    lines.push("");
    ...
    if (!primaryBalance) {
      ...
      const amt = Math.abs(primaryBalance.balance).toLocaleString("en-US", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      });
      lines.push(
        `• حساب ${primaryBalance.currency.name}: ${amt} ${primaryBalance.currency.symbol} (${tag})`,
      );
    }
    lines.push("");
    ...
    <div className="space-y-3 animate-in fade-in duration-300">
      <PersonActionsBar
        onPdf={() =>
          exportPersonStatementPDF({
            ...
            phone,
            txs: scopedTxs,
            currencies: primaryBalance ? [primaryBalance.currency] : currencies,
      ...
      {tab === "timeline" &&
        (loading ? (
          ...
          <EmptyState
            icon={Plus}
            title={`لا توجد معاملات بـ${primaryBalance?.currency.name ?? "هذه العملة"}`}
            description="كل عملة لها مسار مستقل — أضف أول معاملة بهذا المسار."
            variant="compact"
          />
        ) : (
          <PersonTimeline
